...
def pr_insights(owner: str,repo: str,db: Session = Depends(get_db)):
    repo_data = get_repository(db,owner,repo)
    if not repo_data:return {"message":"Repository not found"}
...
def issue_insights(owner: str,repo: str,db: Session = Depends(get_db)):
    repo_data = get_repository(db,owner,repo)
    if not repo_data:return {"message":"Repository not found"}
...
def contributor_insights(owner: str,repo: str,db: Session = Depends(get_db)):
    repo_data = get_repository(db,owner,repo)
    if not repo_data:return {"message":"Repository not found"}
...
@router.get("/analytics/health/{owner}/{repo}")
def repository_health(owner: str,repo: str,db: Session = Depends(get_db)):
    repo_data = get_repository(db,owner,repo)
    if not repo_data:
